(() => {
  ...
  function refreshMenu() {
    ...
    for (const k of Object.keys(SK_CONFIG.iap)) { const el = $("price-" + k), p = SK_CONFIG.iap[k]; if (el && p.price) el.textContent = p.price + (p.subscription ? Lang("perMonth") : ""); }
  ...
  Net.on("input", (p, from) => { if (role !== "host" || !world) return; const pl = world.players.find(x => x.id === from); if (!pl) return; pl.input.dx = p[0]; pl.input.dy = p[1]; pl.input.dash = !!p[2]; });
  Net.on("leave", ({ id }) => {
  ...
  function renderLobby() {
    ...
    const teams = Sim.MODES[lobby.mode].teams; $("btn-dash").textContent = "BOOST";
  ...
  function hostLoopStart() {
    ...
    hostTimer = Net.workerInterval(() => {
      ...
      if (mp) { mp.input.dx = input.dx; mp.input.dy = input.dy; mp.input.dash = dashHeld; }
      Sim.step(world, dt);
  ...
  function snapPayload(d, ev) { return { p: Sim.packPlayers(world), t: Math.round(world.time * 10) / 10, ph: world.phase, cd: world.countdown, ev, food: world.food }; }
  ...
  function clientStart(seed, mode, seconds, players, opts) {
    ...
    remote.clear();
    view = { players: [], food: [], time: seconds, phase: "countdown", countdown: 3 };
    phase = "game"; roundStart = performance.now();
    beginGameUI(); clientLoopStart();
  }
  function applySnap(p) {
    Object.assign(view, { time: p.t, phase: p.ph, countdown: p.cd, food: p.food || [] });
    cw.phase = p.ph; cw.powerups = view.powerups;
    for (const a of p.p) {
      const [id, body, dir, fl, score] = a;
      let r = remote.get(id); if (!r) { r = {}; remote.set(id, r); }
      Object.assign(r, { body, dir, dead: !!(fl & 8), boost: !!(fl & 2), score });
    }
  ...
  function clientLoopStart() {
    ...
    clientTimer = Net.workerInterval(() => {
      ...
      inputAcc += dt; const key = input.dx.toFixed(2) + "," + input.dy.toFixed(2) + (dashHeld ? "b" : "");
      if (key !== lastInputSent || inputAcc >= 0.15) { inputAcc = 0; lastInputSent = key; Net.send("input", [Math.round(input.dx * 100) / 100, Math.round(input.dy * 100) / 100, dashHeld ? 1 : 0], lobby.hostId); }
    }, 1000 / 30);
  }
  ...
  function handleEvents(events) {
    for (const e of events) {
      ...
      else if (e.t === "eat") { if (mine) { Audio2.pickup(); runStats.captures++; } Render.burst(e.x, e.y, "#FFCF5A", 5, 80); }
      else if (e.t === "die") { Render.ring(e.x + 5, e.y + 5, "#fff", 30); Render.burst(e.x + 5, e.y + 5, colorOf(e.id), 16, 180); Audio2.bomb(); if (mine) { Render.kick(0.8); vibrate([40, 30, 60]); } if (e.by === Net.myId) runStats.kos++; if (e.by) Render.popup(e.x + 5, e.y - 10, Lang("ko", nameOf(e.by), nameOf(e.id)), "#fff"); }
  ...
  function buildView() {
    ...
    if (role === "client") {
      ...
      lobby.players.forEach((lp, slot) => {
        ...
        const r = remote.get(lp.id); if (!r) return; players.push({ body: r.body, dir: r.dir, color, name: lp.name, dead: r.dead, boost: r.boost, score: r.score, me: lp.id === Net.myId, meLabel: meLabel() });
      });
      view.players = players; view.showMe = showMe;
      return view;
    }
    return {
      food: world.food, time: world.time, phase: world.phase, countdown: world.countdown, showMe,
      players: world.players.map(p => ({ body: p.body, dir: p.dir, color: Sim.colorOf(world, p), name: p.name, dead: p.dead > 0, boost: p.boost, score: p.score, me: p.id === Net.myId, meLabel: meLabel() })),
    };
  }
  ...
  function frame(now) {
    ...
    } else Render.draw({ players: [], food: [], phase: "idle" }, dt);
    requestAnimationFrame(frame);
  }
  function renderBars() {
    const bars = $("hud-bars");
    const scoreOf = (lp, i) => { if (role !== "client") { const p = world.players[i]; return p ? p.score : 0; } const r = remote.get(lp.id); return r ? r.score || 0 : 0; };
    let rows;
    ...
    const max = Math.max(10, ...rows.map(r => r.v));
  ...
  const keys = {};
  let dashHeld = false;
  window.addEventListener("keydown", (e) => { if (e.target.tagName === "INPUT") return; keys[e.key.toLowerCase()] = true; if (e.key === " ") { e.preventDefault(); dashHeld = true; } updateKeyInput(); });
  window.addEventListener("keyup", (e) => { keys[e.key.toLowerCase()] = false; if (e.key === " ") dashHeld = false; updateKeyInput(); });
  ...
  $("btn-dash").addEventListener("pointerdown", (e) => { e.preventDefault(); dashHeld = true; }); ["pointerup", "pointercancel", "pointerleave"].forEach(ev => $("btn-dash").addEventListener(ev, () => { dashHeld = false; }));
  ...
  function endRound(results) {
    ...
    if (myResult && window.Rating) Rating.add("snakes", Math.round(myResult.coins / 2));
    if (myResult) {
