...
window.Render = (() => {
  ...
  function draw(view, dt) {
    ...
    for (const p of view.players) { if (p.dead || !p.body || !p.body.length) continue;
      ctx.fillStyle = p.color; for (let i = p.body.length - 1; i >= 0; i--) { const c = p.body[i], x = (c % GW) * CELL, y = ((c / GW) | 0) * CELL; const m = i === 0 ? 0.5 : 1.2; ctx.beginPath(); ctx.roundRect ? ctx.roundRect(x + m, y + m, CELL - m * 2, CELL - m * 2, i === 0 ? 4 : 2.5) : ctx.rect(x + m, y + m, CELL - m * 2, CELL - m * 2); ctx.fill(); }
      if (p.boost) { ctx.strokeStyle = "#fff"; ctx.lineWidth = 1.5; const c = p.body[0]; ctx.strokeRect((c % GW) * CELL - 1, ((c / GW) | 0) * CELL - 1, CELL + 2, CELL + 2); }
      const h = p.body[0], hx = (h % GW) * CELL + CELL / 2, hy = ((h / GW) | 0) * CELL + CELL / 2, d = [[1, 0], [0, 1], [-1, 0], [0, -1]][p.dir] || [1, 0];
