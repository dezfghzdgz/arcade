...
window.Render = (() => {
  ...
  function draw(view, dt) {
    ...
    for (const o of view.obstacles) {
      ctx.fillStyle = "#2B2440"; roundRect(o.x, o.y, o.w, o.h, 6);
  ...
  function drawPlayer(p) {
    ...
    if (p.gun) { ctx.save(); ctx.rotate(p.dir); ctx.fillStyle = "#2B2440"; ctx.fillRect(R - 2, -2.5, 9, 5); ctx.fillStyle = "#fff"; ctx.fillRect(R + 4, -1, 3, 2); ctx.restore(); }
    // odlesk
    ...
    ctx.fillStyle = "rgba(43,36,64,.85)"; ctx.fillText(p.name, p.x, p.y - R - 12);
  ...
  function drawPowerupIcon(kind) {
    ...
    else if (kind === "gun") { ctx.fillStyle = dark; ctx.fillRect(-6, -2, 11, 4); ctx.fillRect(-5, 1, 3, 5); ctx.fillStyle = "#FF5E7E"; ctx.beginPath(); ctx.arc(6, 0, 2, 0, 6.28); ctx.fill(); }
