(() => {
  ...
  function drop() {
    ...
    if (off < 7) { combo++; const grow = Math.min(top.w + 8, 200); const nx = top.x - (grow - top.w) / 2; blocks.push({ x: Math.max(0, Math.min(W - grow, nx)), w: grow }); popups.push({ text: L("perfect") + (combo > 1 ? " ×" + combo : ""), y: cur.y, life: 1 }); beep(700 + combo * 60, 0.15); score += 1 + combo; }
  ...
  async function gameOver() { over = true; cur = null; beep(180, 0.4, "sawtooth", 0.15); setTimeout(async () => { $("over").classList.remove("hidden"); $("over-title").textContent = L("gameOver"); $("over-score").textContent = score; $("over-rank").textContent = ""; const r = await Arc.submit("stack", score); if (r) $("over-rank").textContent = L("rank", r); }, 700); }
  function loop(now) {
    ...
    ctx.font = "900 22px Nunito, sans-serif"; ctx.textAlign = "center"; for (const p of popups) { p.life -= dt; ctx.globalAlpha = Math.max(0, p.life); ctx.fillStyle = "#FFCF5A"; ctx.fillText(p.text, W / 2, yOf(p.y) - BH - 10 - (1 - p.life) * 30); } ctx.globalAlpha = 1; popups = popups.filter(p => p.life > 0);
