window.Arc = (() => {
  ...
  async function submit(game, score) { if (window.Rating) Rating.add(game.split("_")[0], score >= 90000 ? 15 : Math.min(30, 5 + Math.floor(Math.max(0, score) / 200))); if (!CFG.supabaseUrl || score <= 0) return null; try { const r = await fetch(CFG.supabaseUrl + "/rest/v1/rpc/submit_score_game", { method: "POST", headers: hdr(), body: JSON.stringify({ p_game: game, p_device: device, p_name: (name || "Player").slice(0, 12), p_score: Math.round(score) }) }); return r.ok ? await r.json() : null; } catch { return null; } }
