...
window.Missions = (() => {
  ...
  const TYPES = {
    ...
    floors:   { mode: "add", d: [[60, 40], [120, 70], [200, 120]], w: [[600, 250], [1200, 400]] },
    best:     { mode: "max", d: [[30, 40], [50, 70], [70, 120]],   w: [[90, 250], [110, 400]] },
    finishes: { mode: "add", d: [[1, 50], [2, 80], [4, 140]],      w: [[10, 250], [20, 400]] },
    jumps:    { mode: "add", d: [[100, 30], [200, 50], [400, 90]], w: [[1500, 150], [3000, 260]] },
  ...
  function afterGame(res, stats) {
    ...
    event("best", res.floor);
    if (res.floor) event("floors", res.floor);
    if (stats.finishes) event("finishes", stats.finishes);
    if (stats.jumps) event("jumps", stats.jumps);
  }
