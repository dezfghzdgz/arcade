(() => {
  ...
  const BUILTIN = [
    ...
    { slug: "pong", title: "Pong", kind: "friends", url: "pong/", icon: "pong/icon-192.png" },
  ...
  const GUEST = [
    { slug: "q1k3", title: "Q1K3", author: "Dominic Szablewski", pc: true },
    { slug: "underrun", title: "UNDERRUN", author: "Dominic Szablewski", pc: true },
    { slug: "packabunchas", title: "PACKABUNCHAS", author: "Rémi Vansteelandt", pc: false },
    { slug: "pathtoglory", title: "PATH TO GLORY", author: "Rémi Vansteelandt", pc: true },
    { slug: "666", title: "666", author: "js13kGames entry", pc: true },
    { slug: "space", title: "SPACE", author: "Matthew Diamant", pc: true },
    { slug: "deathrun", title: "Careless Platformer", author: "Adam Williams", pc: true },
  ];
  function renderGuest() {
    const g = $("guest"); if (!g) return; g.innerHTML = "";
    for (const x of GUEST) { const a = document.createElement("a"); a.className = "card solo" + (x.pc ? " pconly" : ""); a.href = `guest/${x.slug}/`; a.innerHTML = `<span class="tag">${L("guestTag")}</span>${x.pc ? `<span class="tag pcbadge">${L("pcOnly")}</span>` : ""}<h3></h3><p></p><small class="cred"></small>`; a.querySelector("h3").textContent = x.title; a.querySelector("p").textContent = L("g_" + x.slug); a.querySelector("small").textContent = L("by", x.author) + " · js13kGames"; if (x.pc && matchMedia("(pointer: coarse)").matches) { a.classList.add("locked"); a.dataset.lock = L("pcLocked"); a.removeAttribute("href"); } g.appendChild(a); }
  }
  async function renderGames() {
    renderGuest();
    const gs = $("games-solo"), gf = $("games-friends"); gs.innerHTML = ""; gf.innerHTML = "";
    const solo = BUILTIN.filter(g => g.kind === "solo"), friends = BUILTIN.filter(g => g.kind !== "solo");
    // na mobilu (jeden sloupec) střídat sólo/online; na širokém dva sloupce
    const mobile = matchMedia("(max-width: 640px)").matches;
    if (mobile) { const n = Math.max(solo.length, friends.length); for (let i = 0; i < n; i++) { if (friends[i]) gf.appendChild(card(friends[i], false)); if (solo[i]) gf.appendChild(card(solo[i], false)); } }
    else { solo.forEach(g => gs.appendChild(card(g, false))); friends.forEach(g => gf.appendChild(card(g, false))); }
    const add = document.createElement("div"); add.className = "card add"; add.innerHTML = `<div class="plus">+</div><h3></h3><p></p>`; add.querySelector("h3").textContent = L("submitGame"); add.querySelector("p").textContent = L("addHint"); add.onclick = () => { if (!user) { openAuth(); return; } showPage("submit"); }; (mobile ? gf : gs).appendChild(add);
    if (!sb) return;
  ...
  let wasMobile = matchMedia("(max-width: 640px)").matches; window.addEventListener("resize", () => { const m = matchMedia("(max-width: 640px)").matches; if (m !== wasMobile) { wasMobile = m; renderGames(); } });
  applyLang();
})();
