(() => {
  ...
  const BUILTIN = [
    ...
    { slug: "hextris", title: "Hextris", kind: "solo", url: "play.html?url=hextris/&game=hextris", icon: "hextris/icon-192.png", cat: "puzzle", by: "Hextris team", desc: "Rotate the hexagon, match three colors on a side. Keys or tap left/right." },
    { slug: "blocks", title: "Blocks", kind: "solo", url: "play.html?url=blocks/&game=blocks", icon: "blocks/icon-192.png", cat: "puzzle", pc: true, by: "Jake Gordon", desc: "Falling blocks classic. Arrows to move, up to rotate, space to drop." },
  ...
  async function loadShop() {
    ...
    const { data: items } = sb ? await sb.from("shop_items").select("*") : { data: [] }; const grid = $("shop-grid"); grid.innerHTML = "";
    const hasPass = profile && profile.pass_until && new Date(profile.pass_until) > new Date();
    for (const it of (items || [])) { const d = Meta.SHOP[it.item]; if (!d) continue; const owned = (st.inventory || []).includes(it.item), eq = st.equipped && st.equipped[d.slot] === it.item;
      const c = document.createElement("div"); c.className = "shop-item" + (owned ? " owned" : "") + (eq ? " eq" : "");
      c.innerHTML = `<div class="sw" style="background:${d.color || "#3A1A6A"}">${d.icon || ""}</div><b></b><small>${L("slot_" + d.slot)}</small><button class="${owned ? "" : "pass"}">${eq ? L("equipped") : owned ? L("equip") : it.pass_only ? (hasPass ? L("free") : "Pass") : "🪙 " + it.price}</button>`;
  ...
  async function loadProfile() {
    ...
    box.innerHTML = `<div class="ph"><div class="big-lv">${lv}</div><div><h2 id="pf-name"></h2><div class="dim">${Meta.tier(st.xp)} · ${st.xp} XP · ${L("toNext", next - st.xp)}</div><div class="xpbar big"><i style="width:${Math.round(100 * (st.xp - base) / (next - base))}%"></i></div></div></div>
