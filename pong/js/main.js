(() => {
  ...
  function refreshMenu() {
    ...
    for (const k of Object.keys(PG_CONFIG.iap)) { const el = $("price-" + k), p = PG_CONFIG.iap[k]; if (el && p.price) el.textContent = p.price + (p.subscription ? Lang("perMonth") : ""); }
  ...
  function snapPayload(d, ev) { return { p: Sim.packPlayers(world), t: Math.round(world.time * 10) / 10, ph: world.phase, cd: world.countdown, ev, balls: world.balls.map(b => [Math.round(b.x * 10) / 10, Math.round(b.y * 10) / 10, Math.round(b.vx), Math.round(b.vy)]), sc: world.score }; }
  ...
  function clientStart(seed, mode, seconds, players, opts) {
    ...
    m.slot = Math.max(0, idx); m.team = m.slot % 2; m.ci = info.ci; m.y = m.team === 0 ? Sim.H - 30 : 30; m.half = players.length > 2 ? (m.slot < 2 ? 0 : 1) : -1; m.x = m.half < 0 ? Sim.W / 2 : m.half === 0 ? Sim.W / 4 : 3 * Sim.W / 4;
    remote.clear();
    view = { players: [], balls: [], ballsT: [], score: [0, 0], time: seconds, phase: "countdown", countdown: 3 };
    phase = "game"; roundStart = performance.now();
    beginGameUI(); clientLoopStart();
  }
  function applySnap(p) {
    Object.assign(view, { time: p.t, phase: p.ph, countdown: p.cd, score: p.sc || [0, 0] }); view.ballsT = (p.balls || []).map(b => ({ x: b[0], y: b[1], vx: b[2], vy: b[3] })); view.ballsAt = performance.now();
    cw.phase = p.ph; cw.powerups = view.powerups;
    for (const a of p.p) {
      const [id, x, y, pw, fl, g, cd, score] = a;
      if (id === Net.myId) { const m = cw.players[0]; if (Math.abs(m.x - x) > 30) m.x = x; else m.x += (x - m.x) * 0.2; m.y = y; m.w = pw; m.score = score; continue; }
      let r = remote.get(id); if (!r) { r = { x, y, dx: x, dy: y }; remote.set(id, r); }
      Object.assign(r, { tx: x, ty: y, w: pw, score });
    }
  ...
  function clientLoopStart() {
    ...
    clientTimer = Net.workerInterval(() => {
      ...
      input.dash = false; m.input.dx = input.dx; if (cw.phase === "play") Sim.movePlayer(cw, m, dt);
      inputAcc += dt; const key = input.dx.toFixed(2);
      if (key !== lastInputSent || inputAcc >= 0.1) { inputAcc = 0; lastInputSent = key; Net.send("input", [Math.round(input.dx * 100) / 100, 0, 0], lobby.hostId); }
    }, 1000 / 30);
  }
  ...
  function handleEvents(events) {
    for (const e of events) {
      ...
      else if (e.t === "hit") { Render.burst(e.x, e.y, "#fff", 8, 140); if (mine) { Audio2.pickup(); vibrate(10); } else Audio2.tick(); }
      else if (e.t === "wall") { Audio2.tick(); }
      else if (e.t === "goal") { const myTeam = mySlot() % 2; Render.ring(e.x, e.y, Sim.TEAM_COLORS[e.team], 60); Render.burst(e.x, e.y, Sim.TEAM_COLORS[e.team], 30, 220); Render.kick(0.8); if (e.team === myTeam) { Audio2.win(); runStats.kos++; } else { Audio2.bump(); vibrate([40, 30, 60]); } Render.popup(Sim.W / 2, Sim.H / 2, Lang("goalFor", Lang(e.team === 0 ? "teamA" : "teamB")), Sim.TEAM_COLORS[e.team]); }
      else if (e.t === "serve") { Audio2.go(); }
  ...
  function beginGameUI() {
    ...
    $("btn-dash").classList.add("hidden"); $("joy").classList.toggle("hidden", !isTouch);
  ...
  function buildView() {
    ...
    if (role === "client") {
      ...
      lobby.players.forEach((lp, slot) => {
        ...
        if (lp.id === Net.myId) { const m = cw.players[0]; players.push({ x: m.x, y: m.y, w: m.w, color, name: lp.name, team: slot % 2, me: true, meLabel: meLabel() }); }
        else { const r = remote.get(lp.id); if (!r) return; players.push({ x: r.dx, y: r.ty ?? r.dy, w: r.w || T.paddleW, color, name: lp.name, team: slot % 2 }); }
      });
      // míček: extrapolace mezi snapshoty
      const el = (performance.now() - (view.ballsAt || performance.now())) / 1000;
      view.balls = (view.ballsT || []).map(b => ({ x: Math.max(Sim.R, Math.min(Sim.W - Sim.R, b.x + b.vx * el)), y: b.y + b.vy * el }));
      view.players = players; view.showMe = showMe;
      return view;
    }
    return {
      balls: world.balls, score: world.score, time: world.time, phase: world.phase, countdown: world.countdown, showMe,
      players: world.players.map(p => ({ x: p.x, y: p.y, w: p.w, color: Sim.colorOf(world, p), name: p.name, team: p.team, me: p.id === Net.myId, meLabel: meLabel() })),
    };
  }
