...
window.Missions = (() => {
  ...
  const TYPES = {
    ...
    coins:    { mode: "add", d: [[30, 40], [60, 70], [100, 120]],  w: [[300, 250], [600, 400]] },
    best:     { mode: "max", d: [[15, 40], [25, 70], [40, 120]],   w: [[50, 250], [70, 400]] },
    kos:      { mode: "add", d: [[3, 40], [6, 60], [10, 100]],     w: [[25, 200], [50, 350]] },
    flips:    { mode: "add", d: [[30, 30], [60, 50], [120, 90]],   w: [[300, 150], [600, 260]] },
  ...
  function afterGame(res, stats) {
    ...
    event("best", res.own);
    if (stats.coins) event("coins", stats.coins);
    if (stats.kos) event("kos", stats.kos);
    if (stats.flips) event("flips", stats.flips);
